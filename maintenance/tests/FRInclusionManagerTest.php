...
class FRInclusionManagerTest extends PHPUnit_Framework_TestCase {
	/* starting input */
	protected static $inputTmps = array(
	...
	/* output to test against (test# => test) */
	protected static $rOutputTmps = array(
		0 => array( 10, 'XX', 1242 ),
		1 => array( 10, 'YY', 0 ),
		2 => array( 4, 'cite', 30 ),
		3 => array( 4, 'moo', 0 ),
	...
	protected static $rOutputFiles = array(
	...
	protected static $sOutputTmps = array(
		0 => array( 10, 'XX', 1242 ),
		1 => array( 10, 'YY', 0 ),
		2 => array( 4, 'cite', 30 ),
		3 => array( 4, 'moo', 0 ),
	...
	protected static $sOutputFiles = array(
		0 => array( 'fXX', '20100405192110', 'abc1'),
		1 => array( 'fYY', '20000403101300', 'ffc2'),
		2 => array( 'fZZ', '0', ''),
		3 => array( 'fle', '0', ''),
	...
	public function testManagerClear() {
		$im = FRInclusionManager::singleton();
		$im->setReviewedVersions( self::$inputTmps, self::$inputFiles );
		$im->clear();
	...
	public function testReviewedTemplateVersions() {
		$im = FRInclusionManager::singleton();
		$im->setReviewedVersions( self::$inputTmps, self::$inputFiles );
		foreach ( self::$rOutputTmps as $x => $triple ) {
	...
	public function testReviewedFileVersions() {
		$im = FRInclusionManager::singleton();
		$im->setReviewedVersions( self::$inputTmps, self::$inputFiles );
		foreach ( self::$rOutputFiles as $x => $triple ) {
	...
	public function testStableTemplateVersions() {
		...
		$im->setStableVersionCache( self::$inputTmps, self::$inputFiles );
		foreach ( self::$sOutputTmps as $x => $triple ) {
			list($ns,$dbKey,$expId) = $triple;
			$title = Title::makeTitleSafe( $ns, $dbKey );
	...
	public function testStableFileVersions() {
		...
		$im->setReviewedVersions( array(), array() );
		$im->setStableVersionCache( self::$inputTmps, self::$inputFiles );
		foreach ( self::$sOutputFiles as $x => $triple ) {
			list($dbKey,$expTS,$expSha1) = $triple;
			$title = Title::makeTitleSafe( NS_FILE, $dbKey );
